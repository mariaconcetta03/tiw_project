...
public class NewSubfolderServlet extends HttpServlet {
	...
	private void generateHtmlForFolder(PrintWriter out, Folder f, HttpSession session) {
		...
		out.println("<li class=\"folder\"> <a href="+"\"NewSubfolderServlet?action=getChosenFolder&folder=" + token + "\" class=\"highlight\" type=\"submit\">"+ f.nome+" </a>");
				...
				String action = request.getParameter("action");
				 if ("getChosenFolder".equals(action)) { // Gestisci la selezione della cartella 
				  String folder = request.getParameter("folder"); // folder = il token della cartella di destinazione
					...
					// salviamo ID sopracartella nella sessione, poichè ci servirà per la creazione della cartella "figlia"
				  ...
				  session.setAttribute(action, folder);
				 }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		session.setAttribute("idSopracartella", null); // metto a null il valore di idSopracartella, per far apparire il messaggio
...
/*
 * SUGGERIMENTO CHAT SU COME GESTIRE IL CASO IN CUI LA POST DEVE FARE DUE
 * OPERAZIONI DIVERSE: Soluzione: Differenziare le Operazioni Tramite Parametro
 * HTML Usa il parametro action per indicare che l'utente sta selezionando una
 * cartella. Ad esempio:
 * 
 * html Copia codice <ul> <li class="folder"> <a
 * href="YourServlet?action=getChosenFolder&folder=cartella1"
 * class="highlight">Cartella 1</a> </li> <li class="folder"> <a
 * href="YourServlet?action=getChosenFolder&folder=cartella2"
 * class="highlight">Cartella 2</a> </li> <li class="folder"> <a
 * href="YourServlet?action=getChosenFolder&folder=cartella3"
 * class="highlight">Cartella 3</a> </li> </ul> Servlet Modifica la servlet per
 * gestire le diverse azioni in base al valore del parametro action.
 * 
 * java Copia codice import jakarta.servlet.ServletException; import
 ...
 * Ottieni il valore dell'azione String action = request.getParameter("action");
 * 
 * if ("getChosenFolder".equals(action)) { // Gestisci la selezione della
 * cartella String folder = request.getParameter("folder");
 * System.out.println("Cartella selezionata: " + folder);
 * 
 * // Rispondi al client response.setContentType("text/html");
 * response.getWriter().println("<h1>Hai selezionato: " + folder + "</h1>"); }
 * else { // Gestisci altre operazioni con GET
 * response.getWriter().println("<h1>Altra operazione con GET</h1>"); } }
 * 
 * @Override protected void doPost(HttpServletRequest request,
 * HttpServletResponse response) throws ServletException, IOException { //
 * Gestisci operazioni POST (form completo, ecc.)
 * response.getWriter().println("<h1>Operazione POST gestita</h1>"); } }
 * 
 */
